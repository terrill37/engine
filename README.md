...
6. Make sure dependencies are installed (development versions required)
    |---------------------|------------|
    |freetype             |Library used for font operations such as rendering text onto bitmaps|
    |x11                  |Windowing system that provides the basic framework for a GUI environment|
    |xrandr               |Utility used to set the size, orientation or reflection of the outputs for a screen|
    ...
    |ogg                  |Open container format providing efficient streaming and manipulation of digital multimedia|
    |vorbis               |Audio coding format and software reference encoder/decoder for lossy audio compression|
    |vorbisenc, vorbisfile|Libraries for encoding audio in Vorbis format|
    |openal               |Cross-platform audio API that uses coding conventions similar to OpenGL|
    |pthread              |C library for managing threads. Provides an API to perform multithreading|
    |---------------------|------------|
